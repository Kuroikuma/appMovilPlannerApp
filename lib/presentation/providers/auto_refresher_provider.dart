...
class AutoRefresher {
  ...
  void _ejecutar() {
    ...
    if (ubicacionId.isNotEmpty) {
      ref.read(trabajadorNotifierProvider.notifier).cargarTrabajadores(ubicacionId);
      ref.read(registroDiarioNotifierProvider.notifier).cargarRegistros(ubicacionId);
      print("✅ Refresco automático ejecutado con ubicaciónId: $ubicacionId");
    } else {
      print("⚠️ No hay ubicación configurada. Se omite la actualización de trabajadores.");
    }
  }
